...
sealed class DataType<T>(
        ...
        responseParser = {
            buildMap {
                for (i in 0..7) {
                    ...
                    put(bank to sensor, it.toUInt().getBit(i))
                }
            }
        },
    )
    ...
    data object DRIVER_DEMAND_ENGINE_PERCENT_TORQUE : DataType<Percentage>(
        ...
        responseParser = {
            it[0].toInt()
                .minus(125)
                .div(100)
                .toFloat().asValue(Percentage.Unit.PERCENT)
        },
    )
    ...
    data object ENGINE_REFERENCE_TORQUE : DataType<Torque>(
        ...
        expectedBytes = 2,
        responseParser = {
            it.toUShort().toFloat().asValue(Torque.Unit.NEWTON_METER)
        },
    )
    ...
    companion object {
        ...
        private val oxygenSensorAfVParser = { response: UByteArray ->
            ...
            val af = afBytes.toUShort().toFloat()
                .times(2f / 0xFFFF)
            val v = vBytes.toUShort().toFloat()
                .times(8f / 0xFFFF)
                .asValue(Voltage.Unit.VOLT)
        ...
        private val oxygenSensorAfAParser = { response: UByteArray ->
            ...
            val af = afBytes.toUShort().toFloat()
                .times(2f / 0xFFFF)
            val a = aBytes.toUShort().toFloat()
                .div(256)
                .minus(128)
                .asValue(Current.Unit.MILLIAMPERE)
        ...
        private val secondaryOxygenSensorTrimTwoBanksParser = { response: UByteArray ->
            val firstBankFuelTrim = response[0].toInt().div(128f)
                .minus(1)
                .asValue(Percentage.Unit.PERCENT)
            val secondBankFuelTrim = response[1].toInt().div(128f)
                .minus(1)
                .asValue(Percentage.Unit.PERCENT)
