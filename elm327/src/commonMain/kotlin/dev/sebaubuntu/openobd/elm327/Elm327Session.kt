...
import dev.sebaubuntu.openobd.elm327.models.SessionInfo
...
class Elm327Session(
    ...
    suspend fun init() = withContext(dispatcher) {
        ...
        sessionInfo = SessionInfo(
            obdProtocol = socket.executeCommand(DescribeProtocolByNumberCommand).getOrNull(),
            obdProtocolDescription = socket.executeCommand(DescribeProtocolCommand).getOrNull(),
        )

        Logger.info(LOG_TAG) { "ELM327 initialized: $sessionInfo" }
    }
    ...
    /**
     * Get the [SessionInfo] related to this session.
     */
