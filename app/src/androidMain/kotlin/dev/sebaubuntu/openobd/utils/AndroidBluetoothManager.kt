...
class AndroidBluetoothManager(private val context: Context) : BluetoothManager {
    ...
    override fun connection(
        deviceIdentifier: BluetoothDevice.Identifier,
    ) = callbackFlow<ConnectionStatus<BluetoothDevice, BluetoothDevice.Identifier>> {
        // Cancel discovery because it otherwise slows down the connection.
        bluetoothAdapter?.cancelDiscovery()

        val bluetoothDevice = bluetoothAdapter?.getRemoteDevice(deviceIdentifier.macAddress)
        ...
        val bluetoothSocket = runCatching {
            ...
            Logger.warn(LOG_TAG) { "Failed to create RFCOMM socket, trying insecure variant" }
            ...
            runCatching {
                ...
                ConnectionStatus.Connected(
                    device = BluetoothDevice(
                        ...
                        state = BluetoothDevice.State.CONNECTED,
                    ),
                    socket = it.toSocket(),
                )
            }.getOrElse {
    ...
    private fun BluetoothSocket.toSocket() = object : Socket {
        override val inputStream = this@toSocket.inputStream.toModel()
        override val outputStream = this@toSocket.outputStream.toModel()
    }

    private fun android.bluetooth.BluetoothDevice.toModel() = BluetoothDevice(
        deviceIdentifier = BluetoothDevice.Identifier(address),
        displayName = name,
        state = when (
